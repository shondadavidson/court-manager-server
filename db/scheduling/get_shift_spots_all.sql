select *,
 (
    select coalesce(json_agg(s), '[]'::json)
                from (
                    select s.shift_id, r.ref_id, r.first, r.last, r.credentials, r.credentials_verified, s.captain
                    from shift_spots s
                    join referee r on
                    r.ref_id = s.ref_id
                    where s.sid = t.sid and s.referee = true and s.waitlist = false
                    order by captain desc, last, first
                ) s
            ) as signup,
     (
    select coalesce(json_agg(w), '[]'::json)
            ...
            (select count(certified)
                from (
                    ...
                    join referee r on
                    r.ref_id = s.ref_id
                    where s.sid = t.sid and s.waitlist = false and credentials = 'Certified' and captain = false
                ) s
            ) as certified_filled,
            (select count(level_one)
                from (
                    ...
                    join referee r on
                    r.ref_id = s.ref_id
                    where s.sid = t.sid and s.waitlist = false and credentials = 'Level 1' and captain = false
                ) s
            ) as level_one_filled,
            (select count(level_two)
                from (
                    ...
                    where s.sid = t.sid and s.waitlist = false  and credentials = 'Level 2' and captain = false
                ) s
            ) as level_two_filled,
            (select count(none)
                from (
                    ...
                    where s.sid = t.sid and s.waitlist = false  and credentials = 'Not Rated' and captain = false
                ) s
            ) as none_filled,
